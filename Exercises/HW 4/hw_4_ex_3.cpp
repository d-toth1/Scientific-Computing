...
int main() {
    ...
    vector<double> x_obs;
    vector<double> y_obs;
    double x_entry, y_entry;
    bool valid_entry = true;

    cout << "Enter a pair of values, x and y, separated by a space: ";
    while (cin >> x_entry >> y_entry && cin.good()) {
        x_obs.push_back(x_entry);
        y_obs.push_back(y_entry);
        cout << "Enter another pair of values: ";
        cin.clear();
    }
...
void corr_coeff(const vector<double> &x, const vector<double> &y) {
    ...
    if (std_x == 0 || std_y == 0) {
        ...
        cout << "The sample correlation is: " <<  r_sum/double(x.size() - 1.0) << endl;
    }
    return;
}
...
void y_hat(const vector<double> &x, const vector<double> &y) {
    ...
    if (std_x == 0) {
        ...
        m = (r_sum/double(x.size() - 1.0))*(std_y/std_x);
        b = y_ - m*x_;
